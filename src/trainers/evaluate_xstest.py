...
def cos_sim_xstest(model, tokenizer, df, gradient_norms_compare, minus_row, minus_col):
    """
    Evaluate the model on the xstest dataset using
    ...
    pred_labels = [1 if s >= 0.25 else 0 for s in cos_all]
    ...
    print(f"[XSTest] Precision: {precision_val:.4f}, Recall: {recall_val:.4f}, F1: {f1_val:.4f}, AUPRC: {auprc:.4f}")
    return auprc, f1_val